...
class Offer(dict):  
    def __init__(self,
                 ...
                 offer_by: str = None,  # Role of the player who made the offer
                 stamp: int = None,  # Timestamp of the offer
                 from_chat: bool = False, 
                 enhanced: str = None, 
                 profit_bot1: int = None,  # Bot's profit from the offer
                 profit_bot2: int = None,  # User's profit from the offer
                 test: Any = None): 
    ...
    def profits(self, bot_role: str, constraint_user: int, constraint_bot: int):
        ...
        if bot_role == 'supplier':
            ...
            self.profit_bot2 = self.profit_buyer(self.price, self.quality, constraint_user)
        else:  # ROLE_BUYER
            self.profit_bot1 = self.profit_buyer(self.price, self.quality, constraint_bot)
            self.profit_bot2 = self.profit_supplier(self.price, self.quality, constraint_user)
